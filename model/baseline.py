...
def run_baseline(tensors_path="data/processed_tensors.pt"):
    ...
    # FIX: processed_tensors.pt is already RAW/unscaled (dataset_prep.py's own
    # docstring: "Returns raw, UNSCALED tensors"). No inverse_transform is
    # needed here — applying one to already-raw data produced nonsensical
    # values (confirmed via verify_static_baseline.py: sample latencies in
    # the millions of ms). Use the raw last-timestep values directly.
    last_timestep = X_test[:, -1, :].numpy()  # [N, 4] — rps, latency, cpu, mem
    ...
    print(f"{'BiLSTM-Attention (VeloGuard, published)':<35} {'88.78%':>8} {'100.00%':>10} {'94.05%':>8} {'0.9878':>8}")
    print()
    print("Note: Static Threshold row now uses raw (unscaled) last-timestep")
    print("values directly, correcting an earlier version that mistakenly")
    print("applied inverse_transform to already-raw data.")
